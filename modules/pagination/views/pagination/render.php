<div class="pagination">
	<ul>
		...
		<?php if ($page == 1): ?>
			<li class="prev disabled"><a>&larr; сюда</a></li>
		<?php else: ?>
			<li class="prev"><a href="<?php echo Route::url($route, array_merge($route_params, array($route_page_param => $page-1))) . ($_GET ? '?' . http_build_query($_GET) : ''); ?>">&larr; сюда</a></li>
		<?php endif; ?>
		...
		<?php else: ?>
			<li <?php if ($page == 1): ?>class="active"<?php endif; ?>>
				<a href="<?php echo Route::url($route, array_merge($route_params, array($route_page_param => 1))) . ($_GET ? '?' . http_build_query($_GET) : ''); ?>">1
			</a></li>
		...
		<?php if ($page == $pages || $pages == 0): ?>
			<li class="next disabled"><a>туда &rarr;</a></li>
		<?php else: ?>
			<li class="next"><a href="<?php echo Route::url($route, array_merge($route_params, array($route_page_param => $page+1))) . ($_GET ? '?' . http_build_query($_GET) : ''); ?>">туда &rarr;</a></li>
		<?php endif; ?>
