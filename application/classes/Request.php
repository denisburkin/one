...
class Request extends Kohana_Request
{
    ...
    {
        $instance = parent::initial($uri);

        $index_page     = Kohana::$index_file;
        $lang_uri_abbr 	= Kohana::$config->load('keytech.lang_uri_abbr');
        $default_lang 	= Kohana::$config->load('keytech.language_abbr');
        $lang_ignore	= Kohana::$config->load('keytech.lang_ignore');
        ...
        if( ! isset($lang_uri_abbr[$lang_abbr]))
        {
            ...
            if ($lang_abbr != 'admin' AND $cur_lang != $lang_ignore) {
